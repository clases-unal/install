...
**Estado de esta guía:** Fases 1-8 validadas en hardware real (incluyendo la corrección de PATH/variables y el terminal `foot` que faltaba en la Fase 7). Fases 9-11 (Bluetooth/monitores/snapshots, aplicaciones de escritorio, Wlogout) redactadas con paquetes confirmados contra los repositorios de Trixie — pendientes de validar en tu hardware. Fase 12 (perfiles de energía con TLP) recién añadida, también pendiente de validar. El subvolumen `@docker` y el hostname editable se sumaron retroactivamente a las Fases 2 y 4. La Fase 13 (personalización: menús, temas, protector de pantalla animado y atajos) sigue en discusión activa y no está escrita todavía; la de verificación final tampoco.
...
Separa `/`, `/home`, `/var/log`, `/var/cache`, `/var/tmp`, `/opt`, `/var/spool`, `/var/lib/libvirt` y `/var/lib/docker` en subvolúmenes independientes, lo que facilita snapshots selectivos más adelante (Snapper solo necesita snapshotear `@`, dejando fuera tu trabajo y tus datos de VMs/contenedores).
...
Deben aparecer los 9 subvolúmenes, todos los puntos de montaje bajo `/mnt`, y las cuatro variables con valores no vacíos (si alguna sale vacía aquí, corrígela ahora — es mucho más fácil que descubrirlo en la Fase 4).
...
Primero, **edita únicamente esta línea** con el nombre que quieras darle a la máquina (igual que `$DISK` en la Fase 1 — este bloque no lleva nada más):

```bash
export HOSTNAME="debian"
```

El resto se copia y pega tal cual:
...
Tres perfiles de energía (batería, intermedio, rendimiento) con umbrales de carga 30-90%, cambiables con un script cada uno — sin `tlp-rdw` (control manual de radios, como pediste).

### Qué logra esta fase

TLP como único gestor de energía del sistema (sin `power-profiles-daemon` corriendo a la par, para que no compitan por el mismo control), y una carpeta con un script por perfil que sobreescribe la config de TLP y la aplica. El Centro de Control (Fase 13) simplemente ejecutará estos scripts — la lógica de energía vive aquí, no en el menú.
...
`/etc/tlp.d/99-perfil.conf` (lo sobreescribe el script del perfil activo), `~/.config/power-profiles/*.sh`.
...
- Si los archivos `charge_control_*_threshold` no existen en `/sys/class/power_supply/BAT0/`, tu batería/firmware no expone umbrales por `thinkpad_acpi` de la forma esperada — lo revisamos en el momento si pasa, sin asumir de antemano que va a fallar.
- Si `power-profiles-daemon` no estaba instalado, el `systemctl mask` simplemente no hace nada (por eso el `|| true`); no es un error.
